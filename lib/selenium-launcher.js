...
var override = process.env.SELENIUM_VERSION ? process.env.SELENIUM_VERSION.split(':') : []
  ...
  // Only download selenium once when doing concurrent tests
  , concurrentSessions
...
function download(cb) {
  var real = function() {
    ...
    request(requestOptions)
      .on('end', function() {
        ...
        concurrentSessions.forEach(function(callback) {
          callback()
        })
        cb()
      })
  ...
  if (Array.isArray(concurrentSessions)) {
    concurrentSessions.push(cb)
  } else {
